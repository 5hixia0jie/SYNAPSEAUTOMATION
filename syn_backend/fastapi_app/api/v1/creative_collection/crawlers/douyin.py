                ...
                try:
                    with sync_playwright() as p:
                        ...
                        try:
                            ...
                            # 等待页面加载完成
                            page.wait_for_selector("#root", timeout=40000)  # 增加等待时间到40秒
                            
                            # 等待额外的时间确保页面完全加载
                            page.wait_for_timeout(2000)  # 等待2秒

                            logger.info(f"抖音视频页面加载完成: {page.url}")

                            true_url = page.url

                            # 访问页面，使用更宽松的等待策略
                            page.goto(true_url, wait_until="domcontentloaded")  # 使用domcontentloaded代替networkidle
                            ...
                            logger.info(f"抖音视频爬取成功: {data.get('title')}")
                            result.append(data)
                        finally:
                            page.close()
                            browser.close()
